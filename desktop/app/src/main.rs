    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut root =
            div()
                ...
                .on_action(cx.listener(|this, _: &OpenKeybindings, _window, cx| {
                    crate::keybindings_window::KeybindingsWindow::open(this.theme(), cx);
                }))
                .on_action(cx.listener(|this, _: &ExportWorkspaceMarkdown, window, cx| {
                    this.export_workspace_as_markdown(window, cx);
                }))
        ...
        if let Some(popover) = self.render_scheme_color_popover(window, cx) {
            root = root.child(popover);
        }
        if self.search_open {
    ...
    vec![
        ...
        Menu {
            ...
            items: vec![
                ...
                MenuItem::separator(),
                MenuItem::action("Export Workspace as Markdown…", ExportWorkspaceMarkdown),
            ],
        },
        Menu {
        ...
        Menu {
            name: "Help".into(),
            items: vec![MenuItem::action("Keyboard Shortcuts", OpenKeybindings)],
        },
    ]
}
...
mod tests {
    ...
    fn non_macos_menu_bar_policy_matches_platform() {
        ...
        assert!(menu_names.iter().any(|name| name == "Help"));
        assert!(menu_names.len() >= 5);
    }
