...
impl SchemeEditor {
    ...
    /// The link the cursor currently sits in (inclusive of either end), as
    /// `(row, byte range in the line, url to open)`. `None` in read-only schemes
    /// (a plain click already opens those) or when the cursor isn't on a link.
    ...
    fn register_link_hitboxes(&mut self, row: usize, line_origin: Point<Pixels>) {
